...
(lexicals, run_lexer) = lexer(r(WS='\\s+'), r(QuotedStr='"([^\\\\"]+|\\\\.)*?"|\'([^\\\\\']+|\\\\.)*?\''), r(Ident='[a-zA-Z_\\u4e00-\\u9fa5][a-zA-Z0-9_\\u4e00-\\u9fa5]*'), l[','], l['%require'], l['%include'], l['%ignore'], ignores=['WS'], reserved_map=ImmutableMap.from_dict({',': 'quote ,', '%ignore': 'quote %ignore', '%require': 'quote %require', '%include': 'quote %include'}), numbering={'BOF': 0, 'EOF': 1, 'quote ,': 2, 'quote %ignore': 3, 'quote %require': 4, 'quote %include': 5, 'WS': 6, 'QuotedStr': 7, 'Ident': 8})
...
def mk_parser(literal, unesc):
    ...
    def parse_START(prim__state, prim__tokens):
        ...
        if lcl_0:
            ...
            lcl_1 = parse_rbnf__list__0(prim__state, prim__tokens)
            _slot_1_check = lcl_1
