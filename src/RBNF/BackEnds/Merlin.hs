...
encodeTokenName s = ("(* " ++ s ++ " *)") <> "token_id_of_" <> (intercalate "_" . map show $ UTF8.encode s)
caseInt InT {tag, outT = RCall _ [InT {outT = RStr s}]} =  pretty $ encodeTokenName s
...
resolveTokens a =
    vsep $ [
        "let" <+>  pretty n' <+> ": int" <+> "Hashtbl.find LexerInfo.prim_token_names" <+> pretty (show n)
...
-- if, while, switch has to resume the context after getting out of a branch.
    ...
    RSwitch test cases defau -> do
        ...
        cases <- forM cases $ \(a, b) ->
            withScope $ do
                a <- resolveMutable a
                b <- resolveMutable b
                return (a, b)
        defau <-
            withScope $ resolveMutable defau
    ...
    return $ case outT of
        ...
        RDecl   n a
            | mutableInfo M.! scopeId ->  vsep [
                    nest 4  $ vsep [ "let" <+> pretty (showVar n) <+>
                    ":" <+> "(" <> ocamlTypeText myty <> ") ref"  <+> "="
        ...
        RPrj a i     -> "(match" <+> ocamlGenM a <+> "with" <+>
                        tupleDoc [
        ...
        RWhile cond body ->
            vsep [
                nest 2 $ vsep [
                    "while" <+> ocamlGenM cond
                    , ocamlGenM body
                ],
                "done"
            ]
        RSwitch test cases defau ->
            align $ vsep [
                "begin"
                , "match" <+> ocamlGenM test <+> "with"
                , vsep ["|" <+>  ocamlGenM case' <+> "->" <+> (nest 2 $ ocamlGenM body)
                        | (case', body) <- cases]
                , "| _ -> " <+> ocamlGenM defau
                , "end"
            ]
        RDef args body ->
            nest 2 $ vsep [
                "fun" <+> tupleDoc (map (pretty . showVar) args) <+> "->"
                , ocamlGenM body
            ]
        RMutual [] exprs -> nest 2 $
            vsep [
                "begin"
                , vsep [
        ...
        RMutual names exprs ->
            let n1     = length names
                ...
                gen i n e =
                    vsep [
                        head i <+> (pretty . showVar) n <+> ":"
                        <+> ocamlTypeText (snd $ tag e) <+> "="
                        , ocamlGenM e
                    ]
        ...
        RVar name
            | scopeId /= 0 && mutableInfo M.! scopeId -> "!" <> (pretty . showVar) name
